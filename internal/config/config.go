...
var KnownSkeletonSteps = map[string]SkeletonStep{
	...
	"chrome-devtools": {
		...
		Description:   "Comprehensive UI validation via Chrome DevTools",
		Prompt:        "Navigate to the running app (use list_pages or navigate_page to the app URL from CONTEXT.md/README). Step 1 — AUTHENTICATION: take_snapshot to check for login/auth. If login exists: (a) check .env for test credentials or try common defaults; (b) if the app supports API keys, create one via UI or API to test authenticated features; (c) fill credentials and submit; (d) take_snapshot to confirm auth. If auth fails after 2 attempts, document and continue with accessible features. Step 2 — FULL UI AUDIT: walk every section, tab, modal, sidebar. For each: take_snapshot, identify interactive elements (buttons, forms, inputs, dropdowns, links), click each, take_snapshot after to verify state change. Step 3 — FORM VALIDATION: for each form: (a) submit empty — verify validation errors; (b) submit invalid data — verify error messages; (c) submit valid data — verify success. Step 4 — CONSOLE AND NETWORK: list_console_messages — report errors/warnings. list_network_requests — flag 4xx/5xx. Step 5 — SCREENSHOTS: take_screenshot fullPage:true of each distinct view. Step 6 — REPORT: numbered summary: (1) pages/views tested, (2) interactions performed, (3) console errors with message text, (4) network failures, (5) broken functionality.",
		ReadOnly:      false,
		PromptVersion: 2,
	},
	"pencil": {
		Label:         "UI Design",
		Description:   "Design wireframes and review UI via Pencil canvas",
		Prompt:        "Use Pencil MCP to design or review the UI for this task. Step 1 — CANVAS SETUP: Use get_editor_state to check current state. If no .pen file exists for this feature, use batch_design to create a new frame named after the feature. Step 2 — WIREFRAME: Use batch_design to lay out the key UI elements described in the task (containers, navigation, forms, buttons, content areas). Use snapshot_layout to verify positioning and detect overlaps. Step 3 — DESIGN TOKENS: Use get_variables to read the project design tokens (colors, spacing, typography). Apply them consistently via set_variables if needed. Step 4 — SCREENSHOT: Use get_screenshot to capture the current design state. Compare against task requirements. Step 5 — CODE GENERATION: Based on the wireframe, generate the frontend component code (HTML/CSS/JS or framework components) that matches the design exactly. Use the design token values from Step 3 for all styling. Step 6 — VALIDATION: After implementing the code, use get_screenshot again to compare the rendered result with the wireframe. Report any visual discrepancies.",
		ReadOnly:      false,
		PromptVersion: 1,
	},
}
...
// Also upgrades existing prompts when the code default has a higher PromptVersion.
// Returns true if any step was attached or upgraded (config needs saving).
func AttachKnownSkeletonSteps(servers map[string]MCPServer) bool {
	changed := false
	for name, mcp := range servers {
		known, ok := KnownSkeletonSteps[name]
		if !ok {
			continue
		}
		if mcp.SkeletonStep == nil {
			mcp.SkeletonStep = &known
			servers[name] = mcp
			changed = true
		} else if mcp.SkeletonStep.PromptVersion < known.PromptVersion {
			mcp.SkeletonStep.Prompt = known.Prompt
			mcp.SkeletonStep.Description = known.Description
			mcp.SkeletonStep.PromptVersion = known.PromptVersion
			servers[name] = mcp
			changed = true
		}
	}
